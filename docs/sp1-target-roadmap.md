...
此外，项目现在还加入了私有输入、公开输入、`ASSERT_EQ`、`POSEIDON2` 和一个小 assembler。它已经不是单纯的算术 demo，而是一个可以证明“我知道私有 preimage”的最小可用学习 zkVM。
...
- SP1 有 public values 模型；我们目前用单个 `out` 表示 public output。
