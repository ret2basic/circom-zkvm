...
## 第二步：把 VM 从算术 demo 扩成小型 guest runtime

完成第一步后，下一阶段再扩展 ISA 和运行时能力：

1. 增加基本指令：`SUB`、`DUP`、`SWAP`、`EQ`、`LT`、`ASSERT`。
...
5. 增加 assembler，把文本程序编译成固定宽度指令数组。
...
- SP1 有完整 guest/program/script/prover/verifier 工程模型；我们目前只有 circuit、reference VM 和测试。
- SP1 通过 program/vkey 绑定程序身份；旧版教学电路没有 program identity。
- SP1 有 public values 模型；旧版电路暴露了完整执行 trace。
- SP1 有内存、控制流、系统调用、递归和生产 prover 生态；我们目前没有这些层。
